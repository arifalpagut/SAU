const { Leave, LeaveType, LeaveBalance, Employee, Department, Position } = require('../models');
...
async function ensureBalance(employeeId, leaveTypeId, year) {
  let balance = await LeaveBalance.findOne({ where: { employeeId, leaveTypeId, year } });
  if (!balance) {
    const leaveType = await LeaveType.findByPk(leaveTypeId);
    if (!leaveType) throw new AppError('Izin turu bulunamadi', 404);
    balance = await LeaveBalance.create({ employeeId, leaveTypeId, year, totalDays: leaveType.defaultDays, usedDays: 0, remainingDays: leaveType.defaultDays });
  }
  return balance;
}
...
module.exports = { createLeaveRequest, listLeaves, approveLeave, rejectLeave, cancelLeave, getLeaveBalance, getLeaveCalendar };
