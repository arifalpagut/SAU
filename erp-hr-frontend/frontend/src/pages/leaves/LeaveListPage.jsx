...
export default function LeaveListPage() {
  ...
  const { data: leaves, loading } = useFetch(() => leaveService.list({ status: status || undefined }), [status]);
      ...
      <div className="flex gap-2">
        {['', 'PENDING', 'APPROVED', 'REJECTED', 'CANCELLED'].map(s => (
          <button key={s} onClick={() => setStatus(s)} className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${status === s ? 'bg-primary-600 text-white' : 'bg-white border border-gray-300 text-gray-700 hover:bg-gray-50'}`}>
            {s === '' ? 'Tümü' : s === 'PENDING' ? 'Bekleyen' : s === 'APPROVED' ? 'Onaylı' : s === 'REJECTED' ? 'Reddedilen' : 'İptal'}
          </button>
        ))}
      </div>
      <Table headers={['Çalışan', 'İzin Türü', 'Başlangıç', 'Bitiş', 'Gün', 'Durum']}>
        {(leaves || []).map((leave) => (
